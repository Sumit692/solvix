...
interface AnalysisOverlayProps {
  ...
  areaAcres?: number;
  metrics?: {
    solarPotentialKwhPerKwYr?: number;
    windSpeedMph?: number;
    systemSizeKw?: number;
...
function formatMetricNumber(value: number | undefined, digits = 0) {
...
export function AnalysisOverlay({
  ...
  const solarPotential = metrics?.solarPotentialKwhPerKwYr ?? 0;
  const windSpeed = metrics?.windSpeedMph ?? 0;
  const systemSizeKw = metrics?.systemSizeKw ?? 0;
  const annualOutputMwh = metrics?.annualOutputMwh ?? 0;
          ...
          >
            <div className="flex gap-3">
              <MetricCard
                ...
                value={formatMetricNumber(solarPotential, 0)}
                suffix="kWh/kW/yr"
              ...
              <MetricCard
                icon={Wind}
                label="Wind Speed"
                value={formatMetricNumber(windSpeed, 1)}
                suffix="mph avg"
              ...
              <MetricCard
                icon={Battery}
                label="System Size"
                value={formatMetricNumber(systemSizeKw, 0)}
                suffix="kW"
              ...
              <MetricCard
                icon={TrendingUp}
                label="Annual Output"
                value={formatMetricNumber(annualOutputMwh, 0)}
                suffix="MWh"
